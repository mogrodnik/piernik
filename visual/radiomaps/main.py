...
def cli_params(argv):
   ...
   for opt, arg in opts:
      if opt in ("-h", "--help"):
         ...
         print("INFO: Adds background %f uG to B_perp" %stg.background_B)
         ...
         stg.one_bin = int(arg)
         print("Will only use content of one %i-th bin." %stg.one_bin)
...
for i_nl in range(stg.N_nulbd):
   ...
   if (stg.one_bin != False): etyspec = etyspec + "_bin" + str(stg.one_bin)
