...
# cJnu= 2.344*(1.60219)^(p-2)*10^(8-24p)*a(p) (stg)
   ...
   I_sum, Q_sum, U_sum, RM_sum, SI, Q, U = [0.] * N_nl, [0.] * N_nl, [0.] * N_nl, [0.], [0.] * np.shape(stg.SI_set)[0], [0.] * N_nl, [0.] * N_nl
...
# Q,U - Stokes parameters.
   eps_I = I.min() * 1.e-6    # scales component used to avoid division by 0 by using minimum of I
   PI = np.sqrt(Q**2 + U**2)
   PI_obs = (PI / (I + eps_I))
