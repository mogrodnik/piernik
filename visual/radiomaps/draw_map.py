...
# X, Y - a mesh of points to locate vectors
   ...
   scale_u = 5. * np.sqrt(2) * stg.scale_vec_maxPD / ( ax.figure.dpi * cellsize ) if stg.use_vec_scaling else scalevec(ax)
...
# vmin_, vmax_ -  minimum i maksimum of the color scale
   ...
   if (stg.print_cnt): ccx = ax.contour(np.linspace(figext[0],figext[1], np.shape(data)[1]), np.linspace(figext[2],figext[3], np.shape(data)[0]), data, levels=10, colors="white", alpha=0.75, linewidths=1.)
