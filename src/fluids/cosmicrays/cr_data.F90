...
#undef VS
   ...
   subroutine cr_species_tables(ncrsp, crness)
      ...
      eCRSP_Z    (1:nicr) = [  -Z_H1,  Z_H1,   Z_C12, Z_Be9, Z_Be10,  Z_N14, Z_O16,  Z_Li7 ]
      ...
      allocate(cr_names(ncrsp), cr_table(nicr), cr_index(nicr), cr_sigma(ncrsp,ncrsp), cr_tau(ncrsp), cr_primary(ncrsp), cr_mass(ncrsp), cr_Z(ncrsp), cr_spectral(ncrsp), cr_gpess(ncrsp))
      cr_names(:)    = ''
      cr_table(:)    = 0
      cr_index(:)    = 0
      cr_sigma(:,:)  = 0.0
